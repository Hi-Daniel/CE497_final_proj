...
def compute_relative_gaze_features(df):
    if all(col in df.columns for col in ['GazeOrigin_X', 'CameraOrigin_X']):
        rel_gaze = df[['GazeOrigin_X', 'GazeOrigin_Y', 'GazeOrigin_Z']].values - \
                   df[['CameraOrigin_X', 'CameraOrigin_Y', 'CameraOrigin_Z']].values
        rel_gaze_mag = np.linalg.norm(rel_gaze, axis=1)
        stability = np.linalg.norm(np.diff(rel_gaze, axis=0), axis=1).mean()
        return {
            'rel_gaze_distance': np.nanmean(rel_gaze_mag),
            'gaze_stability': stability
        }
    else:
        return {'rel_gaze_distance': np.nan, 'gaze_stability': np.nan}

def compute_movement_features(df):
    return {
        'avg_movement_velocity_mps': df['movement_velocity'].mean(),
        'avg_turning_velocity_degps': df['turning_velocity'].mean(),
        'total_action': (df['movement_velocity'] + df['turning_velocity']).sum()
    }